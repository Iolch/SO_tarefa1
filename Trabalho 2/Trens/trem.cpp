...
//Construtor
Trem::Trem(int ID, int x, int y, int velocidade, int maxVelocidade, sem_t * s, int * estado_rc, int * trem_rc){
    this->ID = ID;
    this->x = x;
    this->y = y;
...
void Trem::enter_rc(int rc){
    ...
    if(ID==AZUL && rc==1){
        ...
        if(estado_rc[2]==OCUPADO && estado_rc[4]==OCUPADO && estado_rc[6]==OCUPADO){
            sem_wait(&s[2]);
            sem_post(&s[2]);
        }
    }
    else if(ID==VERMELHO && rc==2){
        if(estado_rc[3]==OCUPADO && estado_rc[5]==OCUPADO && estado_rc[6]==OCUPADO){
            sem_wait(&s[5]);
            sem_post(&s[5]);
        }
        ...
        if(estado_rc[1]==OCUPADO && estado_rc[3]==OCUPADO && estado_rc[6]==OCUPADO){
            sem_wait(&s[1]);
            sem_post(&s[1]);
        }
    }
        ...
        if(estado_rc[0]==OCUPADO && estado_rc[4]==OCUPADO && estado_rc[6]==OCUPADO){
            sem_wait(&s[6]);
            sem_post(&s[6]);
        }
    }
        ...
        if(estado_rc[0]==OCUPADO && estado_rc[2]==OCUPADO && estado_rc[4]==OCUPADO){
            sem_wait(&s[4]);
            sem_post(&s[4]);
        }
    }
    ...
    if(estado_rc[i]==OCUPADO && trem_rc[t]==rc){
        if(ID==AZUL){
            ...
            if(rc==2){
                ...
                if(estado_rc[3]==OCUPADO && estado_rc[6]){
                    sem_wait(&s[3]);
                    sem_post(&s[3]);
                }
            }
